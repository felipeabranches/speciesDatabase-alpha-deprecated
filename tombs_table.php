                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        $sql = 'SELECT t.id AS tombID, t.name AS tomb, t.entity AS determinator, t.date AS t_date, t.specie_count AS s_count, t.note AS note,
                                s.date AS s_date, s.id_unit AS unit, s.name AS spot, s.place AS place, s.entity AS collector, s.latitude AS latitude, s.longitude AS longitude,
                                sp.id AS spID, CONCAT(sp.gender, " ", sp.specie) AS nomenclature,
                                c.name AS campaing
                                FROM camp_tombs AS t
                                INNER JOIN camp_spots AS s
                                ON s.id = t.id_spot
                        ...
                        if($result = mysqli_query($mysqli,$sql))
                        {
                            ...
                            {
                                echo '<tr><td colspan="14">No entries</td></tr>';
                            }
                        }
                        mysqli_close($mysqli);
                        ?>
